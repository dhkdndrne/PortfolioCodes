...
public class State_Input : State
{
	...
	private async UniTaskVoid SwipeBlock(Block sb, Block tb)
	{
		...
		{
			popBlockDataManager.SpecialBlocks.Add((sb.Hex,SpecialBlockType.Super, sb.GetComponent<ISpecialBlockBehaviour>()));
			popBlockDataManager.SwapSet.Add(tb);
			popBlockDataManager.PopSet.Add(sb);
			ChangeState<State_Pop>();
			return;
		}
		if (tSpecialData?.SBlockType is SpecialBlockType.Super)
		{
			popBlockDataManager.SpecialBlocks.Add((tb.Hex,SpecialBlockType.Super, tb.GetComponent<ISpecialBlockBehaviour>()));
