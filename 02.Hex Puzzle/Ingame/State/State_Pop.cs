...
public class State_Pop : State
{
	...
	{
		bool isPop = await PopAllBlock();

		if (isPop) ChangeState<State_Drop>();
		else ChangeState<State_Input>();
	}
	...
	private async UniTask<bool> PopAllBlock()
	{
		...
		// 특수블록 사용
		if (popBlockDataManager.CombinedSBlock.behaviour != null)
		{
			var sBlock = popBlockDataManager.CombinedSBlock;
			
			board.SetCellShadow(true);
			isShadowActived = true;
			
			//범위 체크
			sBlock.behaviour.Execute(sBlock.hex, withinRangeList);
			...
			popBlockDataManager.CombinedSBlock.behaviour = null;
		}
		...
		while (isLoop)
		{
			...
			while (isItemLoop)
			{
				...
				foreach (var block in popSet)
				{
					...
					sb.Execute(block.Hex, withinRangeList);
