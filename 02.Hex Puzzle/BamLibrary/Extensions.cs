...
namespace Bam.Extensions
{
	public static class Extensions
	{
		...
		public static Vector3 ScreenToWorldPoint(SceneView view)
		{
		...
		public static void InvokePrivateMethod(object target, string methodName)
		{
			...
			if (method != null)
			{
				method.Invoke(target, null); // private 메소드 호출
				Debug.Log($"{methodName} method invoked on {target.GetType().Name}");
			}
			else
			{
				Debug.LogWarning($"{methodName} method not found on {target.GetType().Name}");
			}
		}
