...
public class ObjectPoolManager : Singleton<ObjectPoolManager>
{
    ...
    public void RegistNewObjectInPlay(string key,GameObject prefab,int initAmount = 0,int maxAmount = 0)
    {
        if(!dicPool.ContainsKey(key))
        {
            var objectData = AddNewObject(key,prefab);
            Register(objectData);
        }
    }

    private PoolObjectData AddNewObject(string key,GameObject prefab,int initAmount = 0,int maxAmount = 0)
    {
        PoolObjectData newPoolObjectData = new PoolObjectData();
        newPoolObjectData.prefab = prefab;
        newPoolObjectData.key = key;
        newPoolObjectData.SetObjectCount(initAmount,maxAmount);
        
        poolObjDataList.Add(newPoolObjectData);

        return newPoolObjectData;
    }
