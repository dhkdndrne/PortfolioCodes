...
public class CSVReader
{
	...
	public static async UniTask<Dictionary<string, List<(object, object)>>> ReadToDicMultiList(string _file)
	{
		var dic = new Dictionary<string, List<(object, object)>>();
		TextAsset data = Resources.Load(_file) as TextAsset;
		...
		for (var i = 1; i < lines.Length; i++)
		{
			...
			if (count == 0)
				entry = new List<(object, object)>();
			
			for (var j = 1; j < header.Length && j < values.Length; j+=2)
			{
				string value = values[j];
				value = value.TrimStart(TRIM_CHARS).TrimEnd(TRIM_CHARS).Replace("\\", "");
				...
				string value2 = values[j+1];
				value2 = value2.TrimStart(TRIM_CHARS).TrimEnd(TRIM_CHARS).Replace("\\", "");
				
				object finalValue2 = value2;
				if (int.TryParse(value2, out int m))
				{
					finalValue2 = m;
				}
				else if (float.TryParse(value2, out float f))
				{
					finalValue2 = f;
				}
				entry.Add((finalValue,finalValue2));
				count++;
			}

			if (count == Define.MAX_SUMMON_LEVEL -1)
			{
				dic.Add(values[0], entry);
				count = 0;
			}
